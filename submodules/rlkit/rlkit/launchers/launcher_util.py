...
def create_log_dir(
    ...
    if include_exp_prefix_sub_dir:
        log_dir = osp.join(
            base_log_dir, exp_prefix.replace("_", "-"), exp_name)
    else:
...
try:
    ...
    for code_dir in conf.CODE_DIRS_TO_MOUNT:
        CODE_MOUNTS.append(mount.MountLocal(
            local_dir=code_dir, pythonpath=True))
...
def run_experiment(
    ...
    if use_gpu and mode == 'ec2':
        ...
        if region == 'us-east-1':
            avail_zone = conf.REGION_TO_GPU_AWS_AVAIL_ZONE.get(
                region, "us-east-1b")
