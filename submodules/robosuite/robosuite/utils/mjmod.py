...
class LightingModder(BaseModder):
    ...
    def __init__(
        ...
        super().__init__(env, random_state=random_state)

        if light_names is None:
            light_names = self.env.sim.model.light_names
    ...
    def get_pos(self, name):
        ...
        return copy.copy(self.env.sim.model.light_pos[lightid])
    ...
    def get_dir(self, name):
        ...
        return copy.copy(self.env.sim.model.light_dir[lightid])
    ...
    def get_active(self, name):
        ...
        return copy.copy(self.env.sim.model.light_active[lightid])
    ...
    def get_specular(self, name):
        ...
        return copy.copy(self.env.sim.model.light_specular[lightid])
    ...
    def get_ambient(self, name):
        ...
        return copy.copy(self.env.sim.model.light_ambient[lightid])
    ...
    def get_diffuse(self, name):
        ...
        return copy.copy(self.env.sim.model.light_diffuse[lightid])
    ...
    def get_lightid(self, name):
        ...
        """
        return copy.copy(self.env.sim.model.light_name2id(name))
...
class CameraModder(BaseModder):
    ...
    def __init__(
        ...
        if camera_names is None:
            camera_names = copy.copy(self.env.sim.model.camera_names)
    ...
    def get_fovy(self, name):
        ...
        return copy.copy(self.env.sim.model.cam_fovy[camid])
    ...
    def get_quat(self, name):
        ...
        camid = self.get_camid(name)
        assert camid > -1, "Unknown camera %s" % name
        return copy.copy(self.env.sim.model.cam_quat[camid])
    ...
    def get_pos(self, name):
        ...
        camid = self.get_camid(name)
        assert camid > -1, "Unknown camera %s" % name
        return copy.copy(self.env.sim.model.cam_pos[camid])
    ...
    def get_camid(self, name):
        ...
        """
        return copy.copy(self.env.sim.model.camera_name2id(name))
...
class TextureModder(BaseModder):
    ...
    def __init__(
        ...
        if geom_names is None:
            geom_names = copy.copy(self.env.sim.model.geom_names)
    ...
    def whiten_materials(self):
        ...
        for name in self.geom_names:
            ...
            self.env.sim.model.geom_rgba[geom_id, :] = 1.0
    ...
    def get_geom_rgb(self, name):
        ...
        """
        geom_id = self.env.sim.model.geom_name2id(name)
        return self.env.sim.model.geom_rgba[geom_id, :3]
    ...
    def set_geom_rgb(self, name, rgb):
        ...
        """
        geom_id = self.env.sim.model.geom_name2id(name)
        self.env.sim.model.geom_rgba[geom_id, :3] = rgb
    ...
    def get_material(self, name):
        ...
        material = np.array((self.env.sim.model.mat_reflectance[mat_id],
                             self.env.sim.model.mat_shininess[mat_id],
                             self.env.sim.model.mat_specular[mat_id]))
        return material
    ...
    def set_material(self, name, material, perturb=False):
        ...
        self.env.sim.model.mat_reflectance[mat_id] = material[0]
        self.env.sim.model.mat_shininess[mat_id] = material[1]
        self.env.sim.model.mat_specular[mat_id] = material[2]
...
class DynamicsModder(BaseModder):
    ...
    def __init__(
        ...
        for body_name in self.env.sim.model.body_names:
            ...
            if self.env.sim.model.body_mass[body_id] == 0:
                self.dummy_bodies.add(body_name)

        # Get all values to randomize
        self.body_names = list(
            self.env.sim.model.body_names) if body_names is None else body_names
        self.geom_names = list(
            self.env.sim.model.geom_names) if geom_names is None else geom_names
        self.joint_names = list(
            self.env.sim.model.joint_names) if joint_names is None else joint_names
    ...
    def save_defaults(self):
        ...
        self.opt_defaults = {
            ...
            {
                "density": self.env.sim.model.opt.density,
                "viscosity": self.env.sim.model.opt.viscosity,
            }
        }
        ...
        for body_name in self.env.sim.model.body_names:
            ...
            self.body_defaults[body_name] = {
                "position": np.array(self.env.sim.model.body_pos[body_id]),
                "quaternion": np.array(self.env.sim.model.body_quat[body_id]),
                "inertia": np.array(self.env.sim.model.body_inertia[body_id]),
        ...
        for geom_name in self.env.sim.model.geom_names:
            ...
            self.geom_defaults[geom_name] = {
                "friction": np.array(self.env.sim.model.geom_friction[geom_id]),
                "solref": np.array(self.env.sim.model.geom_solref[geom_id]),
                "solimp": np.array(self.env.sim.model.geom_solimp[geom_id]),
            }

        self.joint_defaults = {}
        for joint_name in self.env.sim.model.joint_names:
            joint_id = self.env.sim.model.joint_name2id(joint_name)
            dof_idx = [i for i, v in enumerate(
                self.env.sim.model.dof_jntid) if v == joint_id]
            self.joint_defaults[joint_name] = {
                "stiffness": self.env.sim.model.jnt_stiffness[joint_id],
                "frictionloss": np.array(self.env.sim.model.dof_frictionloss[dof_idx]),
                "damping": np.array(self.env.sim.model.dof_damping[dof_idx]),
                "armature": np.array(self.env.sim.model.dof_armature[dof_idx]),
            }
    ...
    def randomize(self):
        ...
        for group_defaults, group_randomizations, group_randomize_names in zip(
                (self.opt_defaults, self.body_defaults,
                 self.geom_defaults, self.joint_defaults),
                (self.opt_randomizations, self.body_randomizations,
                 self.geom_randomizations, self.joint_randomizations),
            ...
            for name in group_randomize_names:
                ...
                for attr, default_val in group_defaults[name].items():
                    ...
                    if settings["randomize"]:
                        ...
                        perturbation = np.random.rand() if type(
                            val) in {int, float} else np.random.rand(*val.shape)
                        perturbation = settings["perturbation"] * \
                            (-1 + 2 * perturbation)
                        val = val + \
                            perturbation if settings["type"] == "size" else val * (
                                1.0 + perturbation)
    ...
    def mod_density(self, name=None, val=0.0):
        ...
        # Modify this value
        self.env.sim.model.opt.density = val
    ...
    def mod_viscosity(self, name=None, val=0.0):
        ...
        # Modify this value
        self.env.sim.model.opt.viscosity = val
    ...
    def mod_position(self, name, val=(0, 0, 0)):
        ...
        self.env.sim.model.body_pos[body_id] = np.array(val)
    ...
    def mod_quaternion(self, name, val=(1, 0, 0, 0)):
        ...
        # Modify this value
        body_id = self.env.sim.model.body_name2id(name)
        self.env.sim.model.body_quat[body_id] = val
    ...
    def mod_inertia(self, name, val):
        ...
        if name not in self.dummy_bodies:
            body_id = self.env.sim.model.body_name2id(name)
            self.env.sim.model.body_inertia[body_id] = np.array(val)
    ...
    def mod_mass(self, name, val):
        ...
        if name not in self.dummy_bodies:
            body_id = self.env.sim.model.body_name2id(name)
            self.env.sim.model.body_mass[body_id] = val
    ...
    def mod_friction(self, name, val):
        ...
        # Modify this value
        geom_id = self.env.sim.model.geom_name2id(name)
        self.env.sim.model.geom_friction[geom_id] = np.array(val)
    ...
    def mod_solref(self, name, val):
        ...
        # Modify this value
        geom_id = self.env.sim.model.geom_name2id(name)
        self.env.sim.model.geom_solref[geom_id] = np.array(val)
    ...
    def mod_solimp(self, name, val):
        ...
        # Modify this value
        geom_id = self.env.sim.model.geom_name2id(name)
        self.env.sim.model.geom_solimp[geom_id] = np.array(val)
    ...
    def mod_stiffness(self, name, val):
        ...
        jnt_id = self.env.sim.model.joint_name2id(name)
        if self.env.sim.model.jnt_stiffness[jnt_id] != 0:
            self.env.sim.model.jnt_stiffness[jnt_id] = val
    ...
    def mod_frictionloss(self, name, val):
        ...
        jnt_id = self.env.sim.model.joint_name2id(name)
        if self.env.sim.model.jnt_type[jnt_id] != 0:
            dof_idx = [i for i, v in enumerate(
                self.env.sim.model.dof_jntid) if v == jnt_id]
            self.env.sim.model.dof_frictionloss[dof_idx] = val
    ...
    def mod_damping(self, name, val):
        ...
        if self.env.sim.model.jnt_type[jnt_id] != 0:
            dof_idx = [i for i, v in enumerate(
                self.env.sim.model.dof_jntid) if v == jnt_id]
            self.env.sim.model.dof_damping[dof_idx] = val
    ...
    def mod_armature(self, name, val):
        ...
        if self.env.sim.model.jnt_type[jnt_id] != 0:
            dof_idx = [i for i, v in enumerate(
                self.env.sim.model.dof_jntid) if v == jnt_id]
            self.env.sim.model.dof_armature[dof_idx] = val
