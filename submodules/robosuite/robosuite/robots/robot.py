...
class Robot(object):
    ...
    def __init__(
        ...
        # Specific robot to instantiate
        self.name = robot_type
        self.idn = idn                                      # Unique ID of this robot
        # object holding robot model-specific info
        ...
        if self.initialization_noise is None:
            self.initialization_noise = {
                "magnitude": 0.0, "type": "gaussian"}  # no noise conditions
        ...
        # Base position in world coordinates (x,y,z)
        self.base_pos = None
        # Base rotation in world coordinates (x,y,z,w quat)
        self.base_ori = None
        # xml joint indexes for robot in mjsim
        self._ref_joint_indexes = None
        # xml joint position indexes in mjsim
        self._ref_joint_pos_indexes = None
        # xml joint velocity indexes in mjsim
        self._ref_joint_vel_indexes = None
        # xml joint (torq) actuator indexes for robot in mjsim
        self._ref_joint_actuator_indexes = None

        # Current and last robot arm qpos
        self.recent_qpos = None
        # Current and last action applied
        self.recent_actions = None
        # Current and last torques applied
        self.recent_torques = None
    ...
    def load_model(self):
        ...
        if self.mount_type == "default":
            self.robot_model.add_mount(mount=mount_factory(
                self.robot_model.default_mount, idn=self.idn))
        else:
    ...
    def reset(self, deterministic=False):
        ...
        if not deterministic:
            ...
            if self.initialization_noise["type"] == "gaussian":
                noise = np.random.randn(
                    len(self.init_qpos)) * self.initialization_noise["magnitude"]
            elif self.initialization_noise["type"] == "uniform":
                noise = np.random.uniform(-1.0, 1.0, len(self.init_qpos)) * \
                    self.initialization_noise["magnitude"]
            else:
                raise ValueError(
        ...
        '''
        self.base_ori = T.mat2quat(self.sim.data.get_body_xmat(
    ...
    def pose_in_base_from_name(self, name):
        ...
        pose_in_base = T.pose_in_A_to_pose_in_B(
            pose_in_world, world_pose_in_base)
        return pose_in_base
