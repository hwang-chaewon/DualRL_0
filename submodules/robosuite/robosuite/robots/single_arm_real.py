...
class SingleArmReal(Manipulator):
    """
    ...
    def reset(self, deterministic=False):
        ...
        self.torques = self.controller.torque_compensation
    ...
    def control(self, action, policy_step=False):
        ...
        # Saturate torque rates
        if not self.torques.shape[0] == 7:
            raise "No torques applied"
        torque_differences = torques - self.torques
        clipped_differences = np.clip(
            torque_differences, -self.delta_tau_max_, self.delta_tau_max_)
        self.torques = torques + clipped_differences
