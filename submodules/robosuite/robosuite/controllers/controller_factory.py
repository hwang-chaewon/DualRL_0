...
from .interpolators.position_filter_interpolator import FilterInterpolator
...
def controller_factory(name, params):
    ...
    if params["interpolation"] == "linear":
        interpolator = LinearInterpolator(ndim=params["ndim"],
                                          controller_freq=(
                                              1 / params["sim"].model.opt.timestep),
                                          policy_freq=params["policy_freq"],
                                          ramp_ratio=params["ramp_ratio"])
    if params["interpolation"] == "filter":
        interpolator = FilterInterpolator(
            6, params["policy_freq"], 1 / params["sim"].model.opt.timestep, 1000, params["ramp_ratio"])
    ...
    if name == "OSC_POS_VEL":
        if interpolator is not None:
            interpolator.set_states(dim=6)
        params["control_ori"] = True
        return OSCPosVel(interpolator_pos=interpolator, **params)

    if name == "OSC_POSITION":
        if interpolator is not None:
            # EE control uses dim 3 for pos
            interpolator.set_states(dim=3)
