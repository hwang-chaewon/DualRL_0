...
class OSCPosVel(Controller):
    ...
    def __init__(self,
        ...
        self.current_interp_start = np.array(self.initial_ee_pos)
        self.current_interp_end = np.array(self.initial_ee_pos)
        self.interpolator_pos.set_states(
            start=np.concatenate([self.initial_ee_pos, np.zeros(3)]))
    ...
    def set_goal(self, action, set_pos=None, set_ori=None):
        ...
        delta = scaled_action[:3]
        desired_velocity = scaled_action[3:]

        self.goal_pos = set_goal_position(delta,
                                          self.goal_pos,
                                          position_limit=self.position_limits,
                                          )
        self.goal_vel = desired_velocity

        if self.interpolator_pos is not None:
            self.interpolator_pos.set_goal(
                np.concatenate([self.goal_pos, self.goal_vel]))
            self.current_interp_start = self.interpolator_pos.start
            self.current_interp_end = self.interpolator_pos.target
    ...
    def run_controller(self):
        ...
        # Update state
        self.update()
        desired_pos, desired_vel = self.interpolator_pos.get_interpolated_goal()
        ...
        vel_pos_error = desired_vel - self.ee_pos_vel
        ...
        # TODO: clip torques based on tau_J_d if necessary
        return self.torques
