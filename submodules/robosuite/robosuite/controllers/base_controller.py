...
class Controller(object, metaclass=abc.ABCMeta):
    ...
    def scale_action(self, action):
        ...
        if self.action_scale is None:
            ...
            self.action_output_transform = (
                self.output_max + self.output_min) / 2.0
            self.action_input_transform = (
                self.input_max + self.input_min) / 2.0
        action = np.clip(action, self.input_min, self.input_max)
        transformed_action = (action - self.action_input_transform) * \
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            # self.sim.forward() No need to do this
            ...
            self.J_pos = np.array(self.sim.data.get_site_jacp(
                self.eef_name).reshape((3, -1))[:, self.qvel_index])
            self.J_ori = np.array(self.sim.data.get_site_jacr(
                self.eef_name).reshape((3, -1))[:, self.qvel_index])
            ...
            mujoco_py.cymj._mj_fullM(
                self.sim.model, mass_matrix, self.sim.data.qM)
    ...
    def torque_compensation(self):
        """
        Gravity/coriolis/etc compensation for this robot arm
